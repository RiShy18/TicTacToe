...
(define (candidatos matrix i j lista)
  (cond ((equal? i (len (car matrix))) (candidatos matrix 0 (+ j 1) lista))
        ((equal? j (len matrix)) (reverse lista))
        ((not (or (equal? (get_element matrix j i) 1) (equal? (get_element matrix j i) 2)))
         ( candidatos matrix (+ i 1) j (cons (list (- (posibilidades (replace_matrix matrix j i 2) 1) (posibilidades (replace_matrix matrix j i 2) 2)) i j) lista)))
        (else
         (candidatos matrix (+ i 1) j lista))))

;Busca en la lista de probabilidad y posiciones,
;la tripleta más conveniente para la pc.
(define (seleccion lista)
  (cond ((null? lista) #f)
        (else
         (solucion (car lista) (cdr lista)))))

;Crea una lista con tripletas que contienen (probabilidad i j)
(define (viabilidad matrix)
  (candidatos matrix 0 0 '()))

;verifica si en alguna posición gana el Usuario,
;sustituye temporalmente un espacio y preguntando si ganó el usuario o pc respectivamente,
;luego devuelve esa posición, si alguno gana en ella.
(define (objetivos i j matrix num)
  (cond ((equal? i (len (car matrix))) (objetivos 0 (+ j 1) matrix num))
        ((equal? j (len matrix)) '())
        ((not (or (equal? (get_element matrix j i) 1) (equal? (get_element matrix j i) 2)))
         (cond ((win? (replace_matrix matrix j i num) num)
                (list i j))
               (else
                (objetivos (+ i 1) j matrix num))))
        (else
          (objetivos (+ i 1) j matrix num))))

;Auxiliar que hace todo el milagro,
;lo que hace es buscar cual de las posibilidades calculadas es más negativa.
(define (solucion ele lista)
  (cond ((null? lista) ele)
        ((< (caar lista) (car ele))
         (solucion (car lista) (cdr lista)))
        (else
         (solucion ele (cdr lista)))))





;Funcion de activacion por cola


(define (Gane? matrix num)
  (objetivos 0 0 matrix num))


(define (greedyAlgorithmnAux matrix lista)
  (replace_matrix matrix (cadr lista) (car lista) 2))

;algoritmo principal donde primero se busca el espacio donde puede ganar la pc para colocar la ficha,
;sino,
;pregunta lo mismo para el usuario, para matar una jugada,
;si no,
;calcula la mayor probabilidad de movimientos posibles en el instante para colocar la ficha en ese lugar.
(define (greedyAlgorithmn matrix)
  (cond((not(null? (Gane? matrix 2)))
        (greedyAlgorithmnAux matrix (Gane? matrix 2)))
        ((not(null? (Gane? matrix 1)))
        (greedyAlgorithmnAux matrix (Gane? matrix 1)))
       (else
        (greedyAlgorithmnAux matrix (cdr(seleccion (viabilidad matrix)))))))
